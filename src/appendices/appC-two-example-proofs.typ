...
підрозділах 11.5 і 11.10. Домовленості про позначення, як-от інфіксний запис,
збережено у #(ld)-виведенні, щоб добре бачити математичне підґрунтя.
...
#der((
  ...
  [(6)], [#fl(5)[$"a"_6 (x, u, y, v, w) := "eq-subs"(Z, N, s(x + y), x + s y,$ \
      ...
      #h(2em) $P(x) (s y)$]],
  [(6a)], [#fl(4)[$"a"_(6"a") (x, u, y, v) := arrow.r.double "-in"(P(x) y, P(x) (s y),$ \
      #h(1em) $lambda w : P(x) y . "a"_6 (x, u, y, v, w)) :$ \
      #h(2em) $P(x) y arrow.r.double P(x) (s y)$]],
  [(6b)], [#fl(3)[$"a"_(6"b") (x, u, y) := arrow.r.double "-in"(y epsilon N, P(x) y arrow.r.double P(x) (s y),$ \
      #h(1em) $lambda v : y epsilon N . "a"_(6"a") (x, u, y, v)) :$ \
  ...
  [(7)], [#fl(2)[$"a"_7 (x, u) := forall "-in"(Z, lambda y : Z . (y epsilon N arrow.r.double (P(x) y arrow.r.double P(x) (s y))),$ \
      #h(1em) $lambda y : Z . "a"_(6"b") (x, u, y)) :$ \
      #h(2em) $forall y : Z . (y epsilon N arrow.r.double (P(x) y arrow.r.double P(x)(s y)))$]],
  [(7a)], [#fl(2)[$"a"_(7"a") (x, u) := and "-in"(P(x) 0, forall y : Z . (y epsilon N arrow.r.double (P(x) y arrow.r.double P(x)(s y))),$ \
      #h(1em) $"a"_3 (x, u), "a"_7 (x, u)) :$ \
      #h(2em) $P(x) 0 and forall y : Z . (y epsilon N arrow.r.double (P(x) y arrow.r.double P(x)(s y)))$]],
  [(8)], [#fl(2)[$"a"_8 (x, u) := "nat-ind" (P(x)) "a"_(7"a") (x, u) :$ \
      #h(2em) $forall y : Z . (y epsilon N arrow.r.double x + y epsilon N)$]],
  [(8a)], [#fl(1)[$"a"_(8"a") (x) := arrow.r.double "-in"(x epsilon N, forall y : Z . (y epsilon N arrow.r.double x + y epsilon N),$ \
...
Єдині неповноти, які ми дозволяємо, — це обґрунтування «передзнання», з яким
ми вже мали справу раніше в цій книжці: ці результати ми розглядаємо як
доведені факти.

Ми починаємо зі списку в #(ld)-форматі лем і вправ, які використано в
доведенні, і опускаємо об'єкти доведення. Далі ми даємо розгорнуте доведення
теореми про мінімум як завершення рисунків 15.16–15.18.

Ми користуємося домовленостями про позначення, які описали й застосовували
...
#der((
  ...
  [(5a)], [#fl(6)[$"a"_(5"a") := and "-in"(p x lt.eq x, x lt.eq t, "a"_4, "a"_5) := p x lt.eq x and x lt.eq t$]],
  [(6)], [#fl(6)[$"a"_6 := "Lem-14.10.1.(b)" (p x) x t "a"_(5"a") : p x lt.eq t$]],
  ...
  [(9a)], [#fl(4)[$"a"_(9"a") := and "-in"(exists x : Z . P x,$ \
  ...
  [(10)], [#fl(4)[$"a"_(10) := "Exerc-14.18"(P) "a"_(9"a") : forall x : Z . P x$]],
  [(11)], [#fl(4)[$"a"_(11) := "a"_(10) (s n) : P (s n)$]],
  [(12)], [#fl(4)[$"a"_(12) := "a"_(11) n "ass"_2 : s n lt.eq n$]],
  [(12a)], [#fl(4)[$"a"_(12"a") := "eq-subs"(Z, lambda x : Z . (x epsilon N), n - s n, p(n - n),$ \
      #h(2em) $"Lem-14.8.6.(a)" n n, "a"_(12)) : p(n - n) epsilon N$]],
  [(12b)], [#fl(4)[$"a"_(12"b") := "eq-subs"(Z, lambda x : Z . (p x epsilon N), n - n, 0,$ \
      #h(2em) $"Lem-14.8.4" n, "a"_(12"a")) : p 0 epsilon N$]],
  [(13)], [#fl(4)[$"a"_(13) := "ax-int"_3 "a"_(12"b") : bot$]],
  [(13a)], [#fl(3)[$"a"_(13"a") := lambda "ass"_3 : (forall x : Z . (P x arrow.r.double P (s x))) . "a"_(13) :$ \
      #h(2em) $not forall x : Z . (P x arrow.r.double P (s x))$]],
  [(13b)], [#fl(3)[$"a"_(13"b") := "Fig-11.29" (Z, lambda y : Z . (P y arrow.r.double P (s y))) "a"_(13"a") :$ \
      #h(3em) $exists x : Z . not (P x arrow.r.double P (s x))$]],
  [(13c)], [#fl(3)[$"a"_(13"c") := lambda y : Z . "Exerc-7.5.(b)"(P y, P (s y)) :$ \
  ...
  [(14)], [#fl(3)[$"a"_(14) := "Exerc-7.13" (Z, lambda x : Z . not (P x arrow.r.double P (s x)),$ \
      #h(2em) $lambda z : Z . (P z and not P (s z))) "a"_(13"b") "a"_(13"c") :$ \
